...
adminid='ваш ид!' bd='/mnt/files/test.db' API_TOKEN = '' botan_key = '' 
bot = telebot.TeleBot(API_TOKEN) bot.send_message(adminid,"Я запустился" ) 
logger = telebot.logger 
telebot.logger.setLevel(logging.DEBUG) 
  
  
markup = types.ReplyKeyboardMarkup(row_width=3,resize_keyboard=True) markup2 = types.ReplyKeyboardMarkup(row_width=3,resize_keyboard=True) markup3 = types.ReplyKeyboardMarkup(row_width=2,resize_keyboard=True) markup_h = types.ReplyKeyboardMarkup(row_width=3,resize_keyboard=True) markup_h.add('','','','','\xF0\x9F\x8D\x80 История','', '\xF0\x9F\x93\x9D Рассказать','\xF0\x9F\x94\x99 Назад' ) markup_f = types.ReplyKeyboardMarkup(row_width=3,resize_keyboard=True) markup_f.add('','','','','\xF0\x9F\x8D\x80 Фото','', '','\xF0\x9F\x94\x99 Назад' ) 
markup2.add('\xE2\x9D\xA4 топ 5 фото','\xE2\x9D\xA4 топ 5 историй','','', '\xF0\x9F\x93\x9D Рассказать') markup3.add('Отмена') 
...
markup_start_ne = types.ReplyKeyboardMarkup(row_width=3,resize_keyboard=True) markup_start_ne.add('\xF0\x9F\x8E\xA8 Фото','\xF0\x9F\x93\x9D 
Истории','','Подписаться','\xF0\x9F\x93\x88 Статистика') 
...
@bot.message_handler(commands=['unsubscribe','Stop','stop','off','OFF','Off']) def send_welcome(message): 
     ...
     bot.send_message(message.chat.id,"Отменили подписку! (", reply_markup=markup)      botan.track(botan_key, message.chat.id, message, 'Подписка остановлена', reply_markup=markup_start_ne) 
...
@bot.message_handler(commands=['subscribe']) def send_welcome(message): 
    ...
    global sub     sub = [line.rstrip('\n') for line in open(subfile,'rt')]     if str(message.chat.id) not in sub: 
         ...
         botan.track(botan_key, message.chat.id, message, 'Подписка оформлена')          bot.send_message(adminid,"Подписка оформлена")     else: 
...
"'+str(message.chat.id)+'","'+pa+'","0","0","0")')         con.commit()         id=cur.lastrowid; 
        ...
        img = open(pa, 'rb') 
        callback_bad = types.InlineKeyboardButton(text="Удалить", callback_data=pa)         callback_button = types.InlineKeyboardButton(text="Отстой", callback_data='bad[!!!]'+str(chat_id)) 
        callback_ban = types.InlineKeyboardButton(text="Бан", callback_data='ban[!!!]'+str(chat_id))         callback_send_story = types.InlineKeyboardButton(text="Рассылка", callback_data='sendphoto[!!!]'+str(id)) 
...
@bot.callback_query_handler(func=lambda call: True) def callback_inline(call): 
    global ex     if call.message:         print(call.message)         if '/mnt/files/bot/photo/' in call.data: 
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, caption="Удалил")            except Exception as e: 
...
words = call.data.split("[!!!]")  
            ...
            ex = [line.rstrip('\n') for line in open(banfile,'rt')] 
            bot.answer_callback_query(callback_query_id=call.id, show_alert=True, text="Забанили пользователя: ["+str(words[1])+"]")            except Exception as e: 
...
\xF0\x9F\x98\x87 '+str(data[4])+' \xF0\x9F\x98\xA1 '+str(data[5]);               msg = bot.send_message(line,m, reply_markup=keyboard)    
           ...
           try: 
            ...
            callback_fuu = types.InlineKeyboardButton(text='\xF0\x9F\x98\xA1 фу!', callback_data='ff[!!!]'+str(data[0]))              keyboard.add(callback_shame,callback_nshame,callback_fuu)             for line in sub: 
                          try: 
...
'+str(data[4])+' \xF0\x9F\x98\xA1 '+str(data[5]), reply_markup=keyboard)                   bot.register_next_step_handler(msg, lambda m: process_rating_p(m,data[0],line))              except Exception as e: 
       ...
       if (text in "Отмена"): 
        bot.reply_to(message,"Может в следующий раз )", reply_markup=markup_h)         return        if str(message.chat.id) not in ex:         if len(text)>30 and (len(text)<1001): 
...
"'+str(chat_id)+'","'+text+'","0","0","0")') 
            ...
            callback_bad = types.InlineKeyboardButton(text="Удалить", callback_data='story[!!!]'+str(id))             callback_button = types.InlineKeyboardButton(text="Отстой", callback_data='bad[!!!]'+str(chat_id)) 
            callback_ban = types.InlineKeyboardButton(text="Бан", callback_data='bad[!!!]'+str(chat_id))             callback_send_story = types.InlineKeyboardButton(text="Рассылка", callback_data='sendtext[!!!]'+str(id)) 
            ...
            bot.send_message(adminid,"Добавил: ["+str(chat_id)+"] "+text, reply_markup=keyboard) 
             ...
             bot.reply_to(message, 'История должна быть не менее 20 символов и не более 1000')    except Exception as e:           bot.send_message(adminid,e ) def process_banuser(message):         try: 
            global ex 
...
def process_rating(message,text,ch):     try: 
        ...
        if (unicode(rating)== '\xF0\x9F\x98\x88 стыдно'): 
            ...
            cur.execute("""UPDATE story SET rating = rating + 1  WHERE id="""+str(text)+"""  """)             botan.track(botan_key, message.chat.id, message, 'История стыдно')             con.commit() 
            con.close 
            bot.send_message(ch,'\xF0\x9F\x98\x88') 
 	 	 	 
        elif (rating== '\xF0\x9F\x98\x87 бывает'):             con=lite.connect(bd, timeout=10) 
            cur=con.cursor() 
            cur.execute("""UPDATE story SET be = be + 1  WHERE id="""+str(text)+"""  """)             botan.track(botan_key, message.chat.id, message, ',История бывает')             con.commit()             con.close 
            bot.send_message(ch,'\xF0\x9F\x98\x87', reply_markup=markup)         elif (rating== '\xF0\x9F\x98\xA1 фу!'): 
            con=lite.connect(bd, timeout=10) 
            cur=con.cursor() 
            cur.execute("""UPDATE story SET fu = fu + 1  WHERE id="""+str(text)+"""  """)             botan.track(botan_key, message.chat.id, message, 'История фуу')             con.commit() 
            con.close 
        ...
        rating = message.text         id=rating.replace('#', '')         id=id.replace(' ', '')         id = rating.split('\n')         print str(rating)         if (unicode(rating)== '\xF0\x9F\x98\x88 стыдно'): 
            con=lite.connect(bd)             cur=con.cursor() 
            cur.execute("""UPDATE photo SET rating = rating + 1  WHERE id="""+str(text)+"""  """)             botan.track(botan_key, message.chat.id, message, 'История стыдно')             con.commit()             con.close 
            bot.send_message(ch,'\xF0\x9F\x98\x88') 
 	 	 	 
        elif (rating== '\xF0\x9F\x98\x87 бывает'):             con=lite.connect(bd, timeout=10) 
            cur=con.cursor() 
            cur.execute("""UPDATE photo SET be = be + 1  WHERE id="""+str(text)+"""  """)             botan.track(botan_key, message.chat.id, message, ',История бывает')             con.commit() 
            con.close 
            bot.send_message(ch,'\xF0\x9F\x98\x87')         elif (rating== '\xF0\x9F\x98\xA1 фу!'): 
            con=lite.connect(bd, timeout=10) 
            cur=con.cursor() cur.execute("""UPDATE photo SET fu = fu + 1  WHERE id="""+str(text)+"""  """) botan.track(botan_key, message.chat.id, message, 'История фуу') con.commit()             con.close 
...
@bot.message_handler(func=lambda message: True) def echo_message(message): 
 ...
 try:     global ex 
    ...
    if (str(message.chat.id) in ex): 
       bot.reply_to(message, """Вы попали в бан ((  Потерпите немого""")        botan.track(botan_key, message.chat.id, message, 'Сообщение из бани') 
    
 	      else:         if (message.text in "Подписаться"): 
         ...
         os.system('sed -i "/'+str(message.chat.id)+'/d" '+subfile) 
         bot.send_message(message.chat.id,"Отменили подписку! (", reply_markup=markup_start_ne)          botan.track(botan_key, message.chat.id, message, 'Подписка остановлена')         elif (message.text in '\xF0\x9F\x93\x9D Рассказать')and (len(message.text)>4):           msg = bot.reply_to(message, """Расскажите что случилось?""", reply_markup=markup3)              bot.register_next_step_handler(msg, process_add)         elif ((message.text in "\xF0\x9F\x94\x99 Назад")  and (len(message.text)>3)): 
           sub = [line.rstrip('\n') for line in open(subfile,'rt')]            if str(message.chat.id) not in sub:    
            bot.send_message(chat_id, '\xF0\x9F\x91\x87',reply_markup=markup_start_ne)            else: 
             bot.send_message(chat_id, '\xF0\x9F\x91\x87',reply_markup=markup_start)         elif (message.text== u'Отмена'):  
...
bot.send_message(message.chat.id,'\xF0\x9F\x91\x87', reply_markup=markup_h) 
        elif (message.text in "\xF0\x9F\x8E\xA8 Фото"):             bot.send_message(message.chat.id,'\xF0\x9F\x91\x87', reply_markup=markup_f)         elif ((message.text in "\xF0\x9F\x8D\x80 Фото /photo foto fotka фото")  and (len(message.text)>3)): 
...
\xF0\x9F\x98\x87 '+str(data[4])+' \xF0\x9F\x98\xA1 '+str(data[5]), reply_markup=keyboard)                 bot.register_next_step_handler(msg, lambda m: process_rating_p(m,data[0],message.chat.id)) 
             ...
             botan.track(botan_key, message.chat.id, message, 'Фото')             else: 
                 con=lite.connect(bd, timeout=10) 
                 cur=con.cursor() 
        ...
        elif (message.text in '\xF0\x9F\x93\x88 Статистика'):             con=lite.connect(bd, timeout=10) 
...
'+str(mpb[0][0])+' \xF0\x9F\x98\xA1 '+str(mpf[0][0])             bot.send_message(chat_id,m) 
        ...
        elif (message.text== u'b') and(str(chat_id)==adminid) :             msg = bot.reply_to(message, """Кто идет в бан? """)               bot.register_next_step_handler(msg, process_banuser) 	   
         
        elif (message.text== u'fixphoto') and(str(chat_id)==adminid) :             msg = bot.reply_to(message, """Начинаем фиксить """)               con=lite.connect(bd, timeout=10) 
            cur=con.cursor() 
            ...
            bot.send_message(adminid, "Результат: "+str(con.total_changes))             ex = [line.rstrip('\n') for line in open(banfile,'rt')] 
